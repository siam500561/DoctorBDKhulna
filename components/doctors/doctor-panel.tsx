  ...
  return (
    <div className="group flex flex-col gap-5 rounded-xl border border-border/60 bg-card p-5 transition-colors duration-200 hover:border-border hover:bg-muted/20 sm:flex-row sm:items-start md:p-6">
      <div className="relative shrink-0">
        ...
        <span
          className={`absolute -top-0.5 -right-0.5 size-3 rounded-full border-2 border-card ${doctor.available ? "bg-green-500" : "bg-muted-foreground/30"}`}
        />
      </div>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <span
            className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${doctor.available ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-muted text-muted-foreground"}`}
          >
            {doctor.availability}
          </span>
        </div>
